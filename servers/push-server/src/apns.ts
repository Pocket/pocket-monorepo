import apn, { NotificationAlertOptions, ProviderToken, ResponseFailure, Responses } from 'apn'
import { sqs } from './sqs'
import { TARGET_APNS, TARGET_APNS_SILENT } from './notificationTypes'
import * as config from './config'

const token: ProviderToken = config.apns.token
...
export const apns = {
  sendNotificationToDevice: async (notification: string | NotificationAlertOptions, payload: any, token: string, isSilent: boolean): Promise<void> => {
    ...
    const recipient = apn.token(Buffer.from(tokenString, 'base64'))
    const responses: Responses = await provider.send(note, recipient)

    await Promise.all(responses.failed.map(async (failure: ResponseFailure) => {
      if (failure.status === '410') {
        //This means that the device is unregistered
        console.warn('Device unregistered pushing token to destroy queue', {token})
        await sqs.destroyToken(target, token)
        return
      }
