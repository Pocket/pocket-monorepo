 ...
 * The action performed on the user-item.
 */
export type ListItemUpdateTrigger =
  | 'save'
  | 'archive'
  | 'delete'
  | 'favorite'
 ...
 * The backend action taken that triggers the object update.
 */
export type ObjectUpdateTrigger =
  | 'sso_login'
  | 'account_signup'
  | 'account_product_digest_update'
  | 'account_email_updated'
  | 'account_delete'
  | 'collection_created'
  | 'collection_updated'
  | 'newsletter_signup'
  | 'payment_subscription_renewed'
  | 'payment_subscription_ended'
  ...
  | 'scheduled_corpus_candidate_generated'
 ...
 */
export function createEventSpecification(
 ...
 */
export function trackAPIUser<T extends {} = any>(
  tracker: Tracker,
  aPIUser: APIUser & CommonEventProperties<T>,
) {
 ...
 */
export function trackUser<T extends {} = any>(
  tracker: Tracker,
  user: User & CommonEventProperties<T>,
) {
 ...
 */
export function trackSearchResponseEvent<T extends {} = any>(
  tracker: Tracker,
  searchResponseEvent: SearchResponseEvent & CommonEventProperties<T>,
) {
 ...
 */
export function createSearchResponseEvent(
 ...
 */
export function trackListItemUpdate<T extends {} = any>(
  tracker: Tracker,
  listItemUpdate: ListItemUpdate & CommonEventProperties<T>,
) {
 ...
 */
export function trackListItem<T extends {} = any>(
  tracker: Tracker,
  listItem: ListItem & CommonEventProperties<T>,
) {
 ...
 */
export function trackObjectUpdate<T extends {} = any>(
  tracker: Tracker,
 ...
 * Entity to describe an Item that has been shared from the Pocket Share button.
 */
export function trackPocketShare<T extends {} = any>(
  tracker: Tracker,
  pocketShare: PocketShare & CommonEventProperties<T>,
) {
 ...
 * Event triggered when SearchResults are returned from Pocket&#x27;s search api (for saves and corpus). Entities included: api_user; sometimes user, feature_flag.
 */
export function trackSearchResponseEvent1<T extends {} = any>(
  tracker: Tracker,
  searchResponseEvent1: SearchResponseEvent1 & CommonEventProperties<T>,
) {
 ...
 */
export function createSearchResponseEvent1(
  searchResponseEvent1: SearchResponseEvent1,
) {
  return {
    schema: 'iglu:com.pocket/search_response_event/jsonschema/1-0-4',
 ...
 */
export function trackItemSaveSpec(
  tracker: Tracker,
  itemSave: ListItemUpdate & CommonEventProperties<APIUser | ListItem | User>,
) {
  const eventSpecificationContext: SelfDescribingJson<EventSpecification> =
    createEventSpecification({
      id: '2565192a-4600-45db-861d-d9b9378ed87e',
      name: 'Item Save',
 ...
 */
export function trackCreateShareLinkSpec(
  tracker: Tracker,
  createShareLink: ObjectUpdate &
    CommonEventProperties<APIUser | PocketShare | User>,
) {
  const eventSpecificationContext: SelfDescribingJson<EventSpecification> =
    createEventSpecification({
      id: '3dbe5b4a-333c-4798-83f4-4ef30dfe84be',
  ...
  trackObjectUpdate<APIUser | PocketShare | User>(
    tracker,
    modifiedCreateShareLink,
  );
}
/**
 ...
 */
export function trackUpdateShareLinkSpec(
  tracker: Tracker,
  updateShareLink: ObjectUpdate &
    CommonEventProperties<APIUser | PocketShare | User>,
) {
  const eventSpecificationContext: SelfDescribingJson<EventSpecification> =
    createEventSpecification({
 ...
 */
export function trackSearchResultSpec(
  tracker: Tracker,
  searchResult: SearchResponseEvent1 & CommonEventProperties<APIUser | User>,
) {
  const eventSpecificationContext: SelfDescribingJson<EventSpecification> =
    createEventSpecification({
      id: '8bde42c4-659f-40b6-9a95-865bde7f05c3',
