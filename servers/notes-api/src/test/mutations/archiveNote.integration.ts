...
import { ARCHIVE_NOTE } from '../operations';
...
describe('archiveNote', () => {
  it('archives a note with timestamp', async () => {
    ...
    const res = await request(app)
      .post(graphQLUrl)
      .set({ userid: userId })
      .send({ query: ARCHIVE_NOTE, variables: { input } });
    expect(res.body.errors).toBeUndefined();
    expect(res.body.data?.archiveNote).toMatchObject({
      archived: expect.toBeTrue(),
      updatedAt: now.toISOString(),
    });
  });
  it('archives a note without a timestamp', async () => {
    ...
    const updatedAt = new Date(res.body.data?.archiveNote?.updatedAt);
