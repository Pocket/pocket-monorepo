...
import { CREATE_NOTE_QUOTE } from '../operations';
import { db } from '../../datasources/db';
import { sql } from 'kysely';
import { CreateNoteFromQuoteInput } from '../../__generated__/graphql';
...
describe('note', () => {
  it('creates a note with minimal inputs', async () => {
    const input: CreateNoteFromQuoteInput = {
      quote: JSON.stringify(fromQuote.input),
    };
    const res = await request(app)
      .post(graphQLUrl)
      .set({ userid: '1' })
      .send({ query: CREATE_NOTE_QUOTE, variables: { input } });
    expect(res.body.errors).toBeUndefined();
    expect(res.body.data?.createNoteFromQuote).toMatchObject({
      archived: false,
    ...
    const receivedDoc = res.body.data?.createNoteFromQuote?.docContent;
  ...
  it('throws error for unparseable quote document', async () => {
    const bad = {
      type: 'doc',
      content: [
        {
    ...
    expect(res.body.errors).toBeArrayOfSize(1);
    expect(res.body.errors[0].extensions.code).toEqual('BAD_USER_INPUT');
    expect(res.body.errors[0].message).toMatch('Invalid Document');
  });
});
