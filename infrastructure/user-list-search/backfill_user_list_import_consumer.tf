locals {
  ulib_function_name = "${local.prefix}-UserListImportBackfill"
}

resource "aws_lambda_function" "user_list_import_backfill_sqs_processor" {
...
resource "aws_cloudwatch_log_group" "user_list_import_backfill_sqs_processor" {
  name              = "/aws/lambda/${local.ulib_function_name}"
  retention_in_days = 14
}

resource "aws_lambda_alias" "user_list_import_backfill_sqs_processor" {
  function_name    = aws_lambda_function.user_list_import_backfill_sqs_processor.function_name
  function_version = split(":", aws_lambda_function.user_list_import_backfill_sqs_processor.qualified_arn)[7]
  name             = "DEPLOYED"
  lifecycle {
    ignore_changes = [
...
resource "aws_iam_role" "user_list_import_backfill_lambda_role" {
  name               = "${local.prefix}-UserListImportBackfillLambdaExecutionRole"
  tags               = local.tags
  assume_role_policy = data.aws_iam_policy_document.lambda_assume.json
}

resource "aws_iam_role_policy_attachment" "user_list_import_backfill_lambda_role_xray_write" {
  role       = aws_iam_role.user_list_import_backfill_lambda_role.name
  policy_arn = data.aws_iam_policy.aws_xray_write_only_access.arn
}

resource "aws_iam_role_policy" "user_list_import_backfill_lambda_execution_policy" {
  name   = "${local.prefix}-UserListImportBackfillAccessPolicy"
  role   = aws_iam_role.user_list_import_backfill_lambda_role.id
  policy = data.aws_iam_policy_document.user_list_import_backfill_lambda_execution_policy.json
}

data "aws_iam_policy_document" "user_list_import_backfill_lambda_execution_policy" {
  version = "2012-10-17"
