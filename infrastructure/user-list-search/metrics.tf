    ...
    "sla" = {
      ...
      widgets = [
        ...
        },
        {
          ...
          metrics = [
            local.metrics.elasticsearch.indexing_rate,
            merge(local.metrics.elasticsearch.indexing_latency, { metadata = { yAxis = "right" } })
          ]
        },
        {
          x      = 12.0
          ...
          metrics = [
            local.metrics.elasticsearch.searchable_documents,
            merge(local.metrics.elasticsearch.free_storage, { metadata = { yAxis = "right" } })
          ]
        ...
        },
        {
          ...
          height = 6.0
          properties = {
            title   = "Elasticsearch HTTP Requests"
            stacked = true
            region  = data.aws_region.current.name,
            stat    = "Sum"
            period  = 60
          ...
          metrics = [
            merge(local.metrics.elasticsearch.two_xx_requests, {
              metadata = {
                color = "#2ca02c"
              }
            }),
            merge(local.metrics.elasticsearch.three_xx_requests, {
              metadata = {
                color = "#1f77b4"
              }
            }),
            merge(local.metrics.elasticsearch.four_xx_requests, {
              metadata = {
                color = "#ff7f0e"
              }
            }),
            merge(local.metrics.elasticsearch.five_xx_requests, {
              metadata = {
                color = "#d62728"
              }
            })
