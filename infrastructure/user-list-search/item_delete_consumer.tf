locals {
  id_function_name = "${local.prefix}-ItemDelete"
}

resource "aws_lambda_function" "item_delete_sqs_processor" {
  function_name    = local.id_function_name
...
resource "aws_cloudwatch_log_group" "item_delete_sqs_processor" {
  name              = "/aws/lambda/${local.id_function_name}"
  retention_in_days = 14
}

resource "aws_lambda_alias" "item_delete_sqs_processor" {
  function_name    = aws_lambda_function.item_delete_sqs_processor.function_name
  function_version = split(":", aws_lambda_function.item_delete_sqs_processor.qualified_arn)[7]
  name             = "DEPLOYED"
  lifecycle {
    ignore_changes = [
...
resource "aws_iam_role" "item_delete_lambda_role" {
  name               = "${local.prefix}-ItemDeleteLambdaExecutionRole"
  tags               = local.tags
  assume_role_policy = data.aws_iam_policy_document.lambda_assume.json
}

resource "aws_iam_role_policy_attachment" "item_delete_lambda_role_xray_write" {
  role       = aws_iam_role.item_delete_lambda_role.name
  policy_arn = data.aws_iam_policy.aws_xray_write_only_access.arn
}

resource "aws_iam_role_policy" "item_delete_lambda_execution_policy" {
  name   = "${local.prefix}-ItemDeleteAccessPolicy"
  role   = aws_iam_role.item_delete_lambda_role.id
  policy = data.aws_iam_policy_document.item_delete_lambda_execution_policy.json
}

data "aws_iam_policy_document" "item_delete_lambda_execution_policy" {
  version = "2012-10-17"
